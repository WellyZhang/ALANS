...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_panel_attr_gt(self, object_gt, mask, inconsist_index):
        panel_gt = []
        for i in range(object_gt.shape[0]):
            values = []
            for j in range(object_gt.shape[1]):
                if int(mask[i][j]) == 1:
                    values.append(object_gt[i][j])
            if len(np.unique(values)) == 1:
                panel_gt.append(values[0])
            else:
                panel_gt.append(inconsist_index)
        return panel_gt
    ...
    def __getitem__(self, idx):
        ...
        image = data["image"].reshape(16, 160, 160).astype("uint8")
        target = data["target"]
        ...
        # image_size: 16, slot, img_size, img_size
        ...
        if self.test:
            ...
            rule_gt = np.load(data_path.replace(".npz", "_comp0_rule.npz"))
            exist_indicator = torch.tensor(rule_gt["exist_rule_indicator"],
                                           dtype=torch.long)
            number_indicator = torch.tensor(rule_gt["number_rule_indicator"],
                                            dtype=torch.long)
            exist_rule = torch.tensor(rule_gt["exist_rule"], dtype=torch.long)
            number_rule = torch.tensor(rule_gt["number_rule"],
                                       dtype=torch.long)
            ...
            rule_ret = [
                exist_indicator, number_indicator, exist_rule, number_rule,
                type_rule, size_rule, color_rule
            ]
    ...
    def distribute_four(self, image):
        ...
        resize_part_4 = []
        for idx in range(16):
            resize_part_1.append(
                imresize(part_1[idx, :, :], (self.img_size, self.img_size)))
            resize_part_2.append(
                imresize(part_2[idx, :, :], (self.img_size, self.img_size)))
            resize_part_3.append(
                imresize(part_3[idx, :, :], (self.img_size, self.img_size)))
            resize_part_4.append(
                imresize(part_4[idx, :, :], (self.img_size, self.img_size)))
        ...
        image = np.stack(
            [resize_part_1, resize_part_2, resize_part_3, resize_part_4],
            axis=1)
        return image
    ...
    def distribute_nine(self, image):
        ...
        for idx in range(16):
            ...
            resize_part_4.append(
                imresize(part_4[idx, :, :], (self.img_size, self.img_size)))
            resize_part_5.append(
                imresize(part_5[idx, :, :], (self.img_size, self.img_size)))
            resize_part_6.append(
                imresize(part_6[idx, :, :], (self.img_size, self.img_size)))
            resize_part_7.append(
                imresize(part_7[idx, :, :], (self.img_size, self.img_size)))
            resize_part_8.append(
                imresize(part_8[idx, :, :], (self.img_size, self.img_size)))
            resize_part_9.append(
                imresize(part_9[idx, :, :], (self.img_size, self.img_size)))
        resize_part_1 = np.stack(resize_part_1)
        resize_part_2 = np.stack(resize_part_2)
        resize_part_3 = np.stack(resize_part_3)
        resize_part_4 = np.stack(resize_part_4)
        ...
        image = np.stack([
            resize_part_1, resize_part_2, resize_part_3, resize_part_4,
            resize_part_5, resize_part_6, resize_part_7, resize_part_8,
            resize_part_9
        ],
                         axis=1)
        return image
    ...
    def in_distribute_four_out_center_single(self, image):
        ...
        for idx in range(16):
            resize_part_1.append(
                cv2.dilate(
                    imresize(part_1[idx, :, :],
                             (self.img_size, self.img_size)), kernel))
            resize_part_2.append(
                cv2.dilate(
                    imresize(part_2[idx, :, :],
                             (self.img_size, self.img_size)), kernel))
